...
// Datenbank Cleanup - bedeutet Devices deren Missionen nicht mehr existieren, werden gelöscht und deren Interfaces + Packages

function cleanUpDevices($connection){
    $sql = "SELECT * FROM deploy_vms";
    $result = $connection->query($sql);
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            $vm_id = $row['id'];
            $mission_id = $row['mission_id'];
            $sql = "SELECT * FROM deploy_missions WHERE id = $mission_id";
            $result2 = $connection->query($sql);
            if ($result2->num_rows == 0) {
                // Mission nicht gefunden, lösche VM
                $sql = "DELETE FROM deploy_vms WHERE id = $vm_id";
                ...
                // lösche zugehörige Interfaces
                $sql = "DELETE FROM deploy_interfaces WHERE vm_id = $vm_id";
                ...
                // Lösche zugehörige Packages
                $sql = "DELETE FROM deploy_vm_packages WHERE vm_id = $vm_id";
