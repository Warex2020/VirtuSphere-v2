...
namespace VirtuSphere
{
    ...
    public partial class AnsibleForm : Form
    {
        ...
        private async void btnDeploy2_Click(object sender, EventArgs e)
        {
            ...
            // if selectedPlaybook beinhaltet "createVMs" dann mach msgbox
            if (selectedPlaybook.Contains("createVMs")) {

            // Fragebox
            DialogResult dialogResult = MessageBox.Show("Playbook durchgelaufen und MAC-Adressen in die Datenbank importieren?", "MAC-Adressen in Datenbank importieren", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            // Wenn Dateii ProjectPathTmp + "/vm_mac_list.csv" existiert enable btn_importMacDB
            if (System.IO.File.Exists(ProjectPathTmp + "/vm_mac_list.csv"))
            {
                btn_importMacDB.Enabled = true;
            }
